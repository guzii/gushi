...
namespace NFine.Code
{
    public class WebHelper
    {
        ...
        #region 分页HTML
        public static string PagerHtml(string Htmlstring)
        {
            MatchCollection ma = Regex.Matches(Htmlstring, @"<[p](.[^>]*)>(.*?)</[p]>", RegexOptions.IgnoreCase);
            if (ma.Count > 0)
            {
                double d = (double)ma.Count / 10;
                double Num = Math.Ceiling(d);
                for (int i = 1; i < Num; i++)
                {
                    string PagerHtml = ma[10 * i - 1].Groups[0].Value;
                    Htmlstring = Htmlstring.Replace(PagerHtml, PagerHtml + "_ueditor_page_break_tag_");
                }
            }
            return Htmlstring;
        }
