...
namespace NFine.Code
{
    public class FileHelper
    {
        ...
        public static void AsynAdditionalWrite(string filePath, string content)
        {
            ExistsFile(filePath);
        ...
        #region 异步写入
        public static void AsynWrite(string filePath, string content)
        {
            Parallel.For(0, 10000, e =>
            {
                using (FileStream fs = new FileStream(filePath, FileMode.OpenOrCreate,
                    FileAccess.Write, FileShare.ReadWrite, 1024, false))
                {
