...
namespace NFine.Code
{
    public class Validate
    {
        ...
        #region 数字横线
        public static bool IsValidNumLine(string val)
        {
            return Regex.IsMatch(val, @"^[0-9\-]{1,50}");
        }
        #endregion
        ...
        /// <param name="val"></param>
        /// <returns></returns>
        public static bool IsStringNumber(string val)
        {
            return Regex.IsMatch(val, @"^[A-Za-z_0-9]{1,50}$");
        }
        /// <summary>
        /// 是否Nav地址格式   aaa/page-2
        /// </summary>
        /// <param name="val"></param>
        /// <returns></returns>
        public static bool IsNavUrl(string val)
        {
            var arr = val.Split('/');
            if (arr.Length == 1)
            {
                return IsStringNumber(arr[0]);
            }
            else if (arr.Length == 2)
            {
                var flag = IsStringNumber(arr[0]);
                var pageArr = arr[1].Split('-');
                if (flag && pageArr.Length == 2 && IsValidInt(pageArr[1]))
                    return true;
                else
                    return false;
            }
        ...
        public static bool IsArticleUrl(string val)
        {
            ...
            {
                if (arr[0] != "zt")
                    return IsValidNumLine(arr[1]);
                else
                    return false;
            }
            else if (arr.Length == 3)
            {
                var pageArr = arr[2].Split('-');
                if (arr[0] != "zt" && IsStringNumber(arr[0]) && IsValidNumLine(arr[1])
                    && pageArr.Length == 2 && IsValidInt(pageArr[1]))
                    return true;
                else
                    return false;
            }
        ...
        public static bool IsSpecialUrl(string val)
        {
            //zt/mingren/page-3
            if (val.StartsWith("zt"))
            {
                var arr = val.Split('/');
                if (arr.Length == 1 && arr[0] == "zt")
                    return true;
                else if (arr.Length == 2)
                    return IsStringNumber(arr[1]);
                else if (arr.Length == 3)
                {
                    var pageArr = arr[2].Split('-');
                    if (arr[0] == "zt" && IsStringNumber(arr[1])
                        && pageArr.Length == 2 && IsValidInt(pageArr[1]))
                        return true;
                    else
                        return false;
